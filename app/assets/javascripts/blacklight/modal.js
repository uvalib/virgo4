...
// inside a modal dialog, should have destinations remain inside the modal --
// but won't trigger a modal if not already in one.  No need to repeat
// selectors from trigger selectors, those will already be preserved. MUST be
// manually prefixed with the modal selector, so they only apply to things
// inside a modal.
Blacklight.modal.preserveLinkSelector = Blacklight.modal.modalSelector +
    ' a[data-blacklight-modal~=preserve]';
...
Blacklight.modal.receiveAjax = function(contents) {
    ...
    var container = $('<div>').append(jQuery.parseHTML(contents))
    .find(Blacklight.modal.containerSelector).first();
...
Blacklight.modal.setupModal = function() {
    ...
    // Register both trigger and preserve selectors in ONE event handler,
    // combining into one selector with a comma, so if something matches BOTH
    // selectors, it still only gets the event handler called once.
    $('body').on('click', Blacklight.modal.triggerLinkSelector + ', ' +
    ...
    // Catch our own custom loaded event to implement
    // data-blacklight-modal=closed
    $('body')
    .on('loaded.blacklight.blacklight-modal', Blacklight.modal.checkCloseModal);

    // we support doing data-dismiss=modal on a <a> with a href for non-ajax
    // use, we need to suppress following the a's href that's there for
    // non-JS contexts.
    $('body').on('click', Blacklight.modal.modalSelector +
        ' a[data-dismiss~=modal]', function(e) {
        e.preventDefault();
    });
};
